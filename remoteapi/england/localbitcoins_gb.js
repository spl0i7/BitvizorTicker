...
const urls = {
    btc : [
        'https://localbitcoins.com/sell-bitcoins-online/GB/national-bank-transfer/.json',
        'https://localbitcoins.com/buy-bitcoins-online/GB/national-bank-transfer/.json'
    ]
};
...
function startFetch() {
    for(let k in urls){
        setTimeout(()=>{getHttp(urls[k]);}, 1000 * 60 * 2 * Math.random());
        setInterval(()=>{
            getHttp(urls[k])
        }, WAIT_TIME);
    }
