...
const urls = {
    btc : [
        'https://localbitcoins.com/sell-bitcoins-online/JPY/national-bank-transfer/.json',
        'https://localbitcoins.com/buy-bitcoins-online/JPY/national-bank-transfer/.json'
    ]
};
...
function startFetch() {
    for(let k in urls){
        setTimeout(()=>{getHttp(urls[k]);}, 1000 * 60 * 4 * Math.random());
        setInterval(()=>{
            getHttp(urls[k])
        }, WAIT_TIME);
    }
